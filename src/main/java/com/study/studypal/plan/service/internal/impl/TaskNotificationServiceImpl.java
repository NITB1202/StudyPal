...
public class TaskNotificationServiceImpl implements TaskNotificationService {
  ...
  @Override
  public void publishPlanUpdatedNotification(UUID userId, Plan plan) {
    PlanUpdatedEvent event =
        PlanUpdatedEvent.builder()
            .userId(userId)
            .planId(plan.getId())
            .planCode(plan.getPlanCode())
            .build();
  ...
  @Override
  public void publishTaskCompletedNotification(UUID userId, Task task) {
    TaskCompletedEvent event =
        TaskCompletedEvent.builder()
            .userId(userId)
            .planId(task.getPlan().getId())
            .taskId(task.getId())
            .taskCode(task.getTaskCode())
            .build();
